...
def plot_mean_and_std_dev_per_question_per_group(df_combined, df_separate, df_sequential, group_name, filename):
	...
	for i in range(num_groups):
		ax.bar(x[i::num_groups * 3] - width, combined_means[i], width, label=f'Combined, {group_name} {group_entries[i]}', yerr=combined_stds[i], capsize=5)
		ax.bar(x[i::num_groups * 3] + width, separate_means[i], width, label=f'Separate, {group_name} {group_entries[i]}', yerr=separate_stds[i], capsize=5)
		ax.bar(x[i::num_groups * 3] + 3*width, sequential_means[i], width, label=f'Sequential, {group_name} {group_entries[i]}', yerr=sequential_stds[i], capsize=5)
